...
let eyeDetectionRadiusSlider = document.getElementById('eyeDetectionRadiusSlider');
...
resizeWorldButton.onclick = async function () {
...
fluidGridSizeSlider.oninput = async function () {
...
if (useGpuFluidToggle) { // Check if the element exists before assigning onchange
    useGpuFluidToggle.onchange = async function () {
        ...
        await initFluidSimulation(USE_GPU_FLUID_SIMULATION ? webgpuCanvas : canvas); // Re-initialize to apply the change
    };
} else {
