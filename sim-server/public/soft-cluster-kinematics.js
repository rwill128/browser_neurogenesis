...
/**
 * Compute per-cluster center-of-mass and effective angular inertia/velocity
 * from the current soft-node state.
 *
 * Returns a Map keyed by clusterId.
 */
export function computeSoftClusterKinematics(nodes, {
  minMass = 0.02,
  minInertia = 1e-4,
} = {}) {
...
/**
 * Blend node velocities toward each cluster's rigid-body velocity field
 * (COM translation + angular term). This preserves spring deformation while
 * making momentum transfer visibly rotational.
 */
export function projectNodesTowardClusterRigidMotion(nodes, clusterKinematics, {
  linearGain = 0.08,
  angularGain = 0.18,
