...
test('gpu-lab routes rigid-soft and soft-soft collision stepping through isolated gpu-only collision iteration orchestrator', () => {
  ...
  assert.match(
    source,
    /if \(solverPath === 'gpu-only'\) \{[\s\S]*await runCollisionIterationsGpuOnly\(\{[\s\S]*resolveRigidSoftCollisionPassGpuOnly,[\s\S]*resolveSoftSoftCollisionPassGpuOnly,[\s\S]*wgslOffload:[\s\S]*enabled: Boolean\(sim\?\.device\),[\s\S]*device: sim\?\.device,[\s\S]*state: \(sim\.collisionBoundaryWgslState \|\|= \{\}\),[\s\S]*\}\);[\s\S]*\} else \{[\s\S]*buildRigidSoftSpatialHashCandidates\([\s\S]*for \(let rbi = 0; rbi < bodies\.rigid\.length; rbi\+\+\)[\s\S]*resolveRigidVsSoftNodeCollision\([\s\S]*resolveSoftNodeVsSoftEdgeCollision\(node, a, b, 0\.12\)/,
