...
test('viscosity landscape defaults expose stronger-shape controls', () => {
  ...
  assert.ok(Number(config.FLUID_STEP_EVERY_N_TICKS) >= 1);
  assert.ok(Number(config.FLUID_ACTIVE_TILE_SIZE_CELLS) >= 1);
...
test('edge-length telemetry defaults are enabled and hard cap defaults on at factor 6+', () => {
  ...
  assert.equal(config.FORCE_ALL_SPRINGS_RIGID, true);
